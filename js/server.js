...
app.use(session({
  ...
  cookie: { secure: isProduction, httpOnly: true, sameSite: 'strict' }
}));
...
// Juste AVANT tes routes (après les middlewares)
app.use((req, res, next) => {
...
app.post('/login', authLimiter, async (req, res) => {
  ...
  try {
    ...
    req.session.regenerate(err => {
      ...
      res.json({ message: 'Login successful.', user: { id: user.id, username: user.username }, redirect: '/jokes' });
    });
  } catch (err) {
    res.status(500).json({ message: 'Internal server error.' });
  }
...
app.get('/profile', isAuthenticated, (req, res) => {
  res.json({ isLoggedIn: true, user: { id: req.session.userId, username: req.session.username } });
});

// --- JOKES PAGE ---
app.get('/jokes', isAuthenticated, async (req, res) => {
  try {
    const jokePromises = Array.from({ length: 10 }, () => fetch('https://api.chucknorris.io/jokes/random').then(r => r.json()));
...
// --- FAVORITES PAGE & API ---
// Remplace temporairement ta route par celle-ci
app.get('/favorites', (req, res) => {
  res.send("Le serveur voit bien la route favorites !");
});
...
app.post('/api/favorites', isAuthenticated, async (req, res) => {
  ...
  if (!joke_id || !joke_text) return res.status(400).json({ message: 'Joke ID & text required.' });
  ...
  try {
    ...
    const result = await db.run('INSERT INTO favorites (user_id, joke_id, joke_text) VALUES (?, ?, ?)', [req.session.userId, joke_id, joke_text]);
...
app.delete('/api/favorites/:joke_id', isAuthenticated, async (req, res) => {
  ...
  try {
    const result = await db.run('DELETE FROM favorites WHERE user_id = ? AND joke_id = ?', [req.session.userId, joke_id]);
    if (result.changes === 0) return res.status(404).json({ message: 'Favorite not found.' });
